#!/bin/bash
#
# Title: CentOS7.*_x64_upgrade_kernel_script
# Author: ZNing
# Date: 2019-7-3 23:13:09
# MOD by cnrock // 2022-10-01
...
function checkSystem()
{
    ...
    uname -sr;
    uname -a;
    echo "[Notice] Confirm Upgrade Kernel? please select: (1~5)"
    select selected in '升级所有的包' '仅升级内核至5.*-lt长期版' '仅升级内核至5.*-ml主线版' '删除旧内核' 'Exit'; do break; done;
	...
	if [ "$selected" == '升级所有的包' ]; then
		...
		upgradeKernel;
        confirmKernel;
	elif [ "$selected" == '仅升级内核至5.*-ml主线版' ]; then
		upgradeKernel-ml;
        confirmKernel-ml;	
