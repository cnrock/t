...
#    -a     Action: list|ls|clean|gc|del|delete, Default list
#    -c     Only useful if action is gc, specify kubeconfig Absolute path(recommend)
...
#    $0 -a gc -c /root/.kube/kubeconfig -d
...
function check_opts_and_args
{
    ...
    [[ "${ACTION}" == "gc" ]] && { KUBECTL="${KUBECTL} --kubeconfig ${KUBECONFIG:-"/root/.kube/config"}";return; } || echo -n
    echo "${ACTION}" | grep -Eq 'list|ls|clean|gc|del|delete' || help_doc
...
while getopts ':a:h:u:p:r:n:c:d' OPT
do
    case ${OPT} in
        a)
            ACTION="${OPTARG}"
...
# action gc delete blobs
function action_gc() {
    log_debug "Action gc mode.."
    log_debug "${KUBECTL} get pod | grep -E 'dce-registry.*Running' | awk '{print \$1}' | head -1"
    dce_registry_pod=$(${KUBECTL} get pod | grep -E 'dce-registry.*Running' | awk '{print $1}' | head -1)
    now=$(date '+%Y-%m-%d-%H-%M-%S')
    log_debug "${KUBECTL} exec -it ${dce_registry_pod} -- registry garbage-collect /etc/docker/registry/conf.yml >> /var/log/img-gc-${now}.log 2>&1"
    ${KUBECTL} exec -it ${dce_registry_pod} -- registry garbage-collect /etc/docker/registry/conf.yml >> /var/log/img-gc-${now}.log 2>&1
    ls -lt /var/log/img-gc-* | awk 'NR>3{print $NF}' | xargs rm -rf
}
